...
// 图片预加载方法
function hsyLazyload(parent) {
	// 获取所有带 hsy-lazyload 的属性元素
	var aImgWj = document.querySelectorAll(parent + ' [hsy-lazyload]');
	for (var i = 0; i < aImgWj.length; i++) {
		(function(m) {
			var img = new Image();
			var nowSrc = aImgWj[m].getAttribute('hsy-lazyload');
			img.src = nowSrc;
			//监听
			img.onload = function() {
				aImgWj[m].src = nowSrc;
